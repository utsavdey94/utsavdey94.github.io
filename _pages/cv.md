---
layout: archive
title: "Curriculum Vitae"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

## Professional Summary

Materials scientist with a Ph.D. in Materials Science and Engineering and more than nine years of research experience spanning polymer science, functional coatings, thin films, semiconductor processing, electrochemistry, and advanced materials. Experienced in materials synthesis, process development, advanced characterization, scale-up, troubleshooting, and establishing structure–property relationships across academic, industrial, and translational R&D environments.

## Education

**Ph.D., Materials Science and Engineering**  
Iowa State University, 2022-2025

**M.S., Inorganic Chemistry**  
Iowa State University, 2019-2021

**M.Sc., Chemistry**  
Banaras Hindu University, 2015-2017

**B.Sc., Chemistry**  
St. Xavier's College, Kolkata, 2012-2015

## Research & Professional Experience

**Iowa State University / Janas Materials Inc.**  
Research and development of functional polymer systems and coatings, including polymer synthesis, emulsion polymerization, formulation development, materials characterization, process optimization, scale-up, and troubleshooting.

**Rust-Oleum Corporation**  
Industrial R&D experience in formulation development and performance optimization of multipurpose surface-cleaning technologies, including surfactant selection, cleaning efficacy, pH optimization, and cost reduction.

**Graduate Research — Iowa State University**  
Research spanning polymeric Janus nanoparticles, antimicrobial and omniphobic coatings, thin-film materials, semiconductor processing, and structure–property relationships.

**Energy Materials Research**  
Research experience in sodium- and lithium-ion battery materials, solid-state synthesis, electrochemical characterization, thermoelectric materials, and related functional inorganic systems.

## Technical Expertise

**Polymer & Coating Technologies:**  
Emulsion polymerization, functional nanoparticles, Janus particles, waterborne coatings, antimicrobial coatings, omniphobic surfaces, surface modification, formulation development

**Thin Films & Semiconductor Processing:**  
Photolithography, oxidation, diffusion, wet processing, plasma etching, metallization, thin-film processing, wafer fabrication, semiconductor process fundamentals

**Materials Characterization:**  
SEM, TEM/EDX, AFM, DLS, zeta potential, GPC, FTIR, DSC, TGA, rheology, contact-angle analysis, mechanical testing, structural and surface characterization

**Electrochemical & Energy Materials:**  
Battery materials, cyclic voltammetry, electrochemical impedance spectroscopy, electrochemical cycling, solid-state materials synthesis, thermoelectric materials

**Process Development & Data Analysis:**  
DOE, SPC, statistical analysis, process optimization, scale-up, troubleshooting, root-cause analysis, and structure–property correlations

## Selected Highlights

- Publications in *Small*, *Langmuir*, *Chemistry of Materials*, *ACS Applied Materials & Interfaces*, *Journal of Colloid and Interface Science*, and *Nanoscale*
- Co-inventor on a U.S. provisional patent related to functional particle and coating technologies
- MRS Best Poster Award
- Iowa State University Graduate College Teaching Excellence Award
- Sci-Mix selections at ACS Spring and Fall Meetings
- Experience across academic research, industrial R&D, and translational materials development
- Peer reviewer for journals published by ACS, RSC, Wiley, Springer, and MDPI
- Experience mentoring and instructing more than 500 undergraduate students

## Professional Service

Peer reviewer for scientific journals including *ACS Omega*, *Langmuir*, *Nanoscale Horizons*, *Analyst*, *ChemPhysChem*, *Journal of Coatings Technology and Research*, *Polymers*, *Molecules*, *Sensors*, and *MRS Communications*.

Invited session moderator at the **Coatings Trends & Technologies Summit 2025**.

## Full Curriculum Vitae

Additional details on my research experience, publications, technical expertise, awards, and professional activities are available upon request.

[**Request Full CV by Email**](mailto:YOUR_EMAIL_ADDRESS?subject=CV%20Request%20-%20Utsav%20Kumar%20Dey)
